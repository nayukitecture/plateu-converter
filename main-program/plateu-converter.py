...
使い方:
  python convert.py --lod 1 --meshes 53390329 53390338 --input <gmlフォルダ> --output <出力先>
  python convert.py --list --input <gmlフォルダ>   # 利用可能なメッシュ番号を表示

出力:
  メッシュ番号ごとに 1 つの OBJ ファイルを生成する。
  ファイル内では建物ごとに "o 建物ID" で区切られているので、
  Rhinoceros で開いて Explode すれば 1 棟ずつのオブジェクトになる。
...
def parse_gml(gml_path, lod, obj_type='bldg'):
    ...
    for root_tag in config['root_tags']:
        for element in root.findall(f'.//{obj_type}:{root_tag}', NS):
            ...
            if obj_type == 'bldg':
                ...
                if lod == 2 and lod2_tag:
                    polys = get_polygons_generic(element, obj_type, lod2_tag)
                    if not polys:
                        polys = get_polygons_generic(element, obj_type, lod1_tag)
                        skipped_lod += 1
                else:
                    polys = get_polygons_generic(element, obj_type, lod1_tag)
...
def write_obj_file(out_path, mesh_buildings, lod, label):
    """
...
例:
  # メッシュ番号一覧を確認
  python convert.py --list --input ../resources/input/14100.../udx/bldg

  # LOD1 で 2 メッシュだけ変換
  python convert.py --lod 1 --meshes 53390329 53390338 --input ../resources/input/14100.../udx/bldg

  # LOD2 で全メッシュ変換
  python convert.py --lod 2 --input ../resources/input/14100.../udx/bldg
""")
    ...
    parser.add_argument('-t', nargs='+', metavar='TYPE', dest='types', default=['bldg'],
                        help='オブジェクト種別（複数指定可、デフォルト: bldg）  例: bldg brid tran luse veg')

    args = parser.parse_args()

    # 出力先フォルダを決定
    base_output = os.path.join(os.path.dirname(__file__), '..', 'resources', 'output')
    output_dir = os.path.join(base_output, args.output) if args.output else base_output
    ...
    for obj_type in args.types:
        ...
        if args.meshes:
            ...
            if not_found:
                print(f'[WARNING] {obj_type}: 見つからないブロック番号: {not_found}')
            if not target:
                print(f'[SKIP] {obj_type}: 変換対象のファイルがありません')
                continue
        else:
            target = all_gml
        ...
        if total_count == 0:
            print(f'[SKIP] {obj_type}: 変換できるデータが 0 件のため出力しません')
            continue
